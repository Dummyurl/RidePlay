package rnf.taxiad.cryptography;
...
public class CryptLib {
    ...
    private String encryptDecrypt(String _inputText, String _encryptionKey,
        ...
        if (_mode.equals(EncryptMode.ENCRYPT)) {
            ...
            // transformation
            // (encryption)
            String enc = new String(results, "UTF-8");
            System.out.println(enc);
            _out = Base64.encodeToString(results, Base64.NO_WRAP);
            _out = Base64.encodeToString(_out.getBytes("UTF-8"), Base64.NO_WRAP);// ciphertext
            // output
//            _out = Base64.encodeToString(_out.getBytes("UTF-8"), Base64.NO_WRAP);
        }
        ...
        if (_mode.equals(EncryptMode.DECRYPT)) {
            ...
            byte[] decodedValue = Base64.decode(_inputText.getBytes("UTF-8"),
                    Base64.NO_WRAP);
            decodedValue = Base64.decode(decodedValue,
                    Base64.NO_WRAP);
//            decodedValue = Base64.decode(decodedValue,
//                    Base64.NO_WRAP);
    ...
    public static synchronized String convert(String text, EncryptMode mode, String key) {
        String finalText = "";
        try {
            final CryptLib cryptLib = new CryptLib();
            switch (mode) {
                case ENCRYPT:
                    finalText = cryptLib.encryptString(text, key);
                    break;
                case DECRYPT:
                    finalText = cryptLib.decryptString(text, key);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private String encryptString(String textToEncrypt, String key) throws Exception {
        final String sha256 = CryptLib.SHA256(key, 32);
        String iv = CryptLib.generateRandomIV(16);
        Log.e(this.getClass().getSimpleName(), "ENCRPTION IV= " + iv);
        final String encryptedText = encrypt(textToEncrypt, sha256, iv);
        final JSONArray jsonArray = new JSONArray();
        jsonArray.put(0, encryptedText);
        jsonArray.put(1, iv);
//        final StringBuffer buffer = new StringBuffer(iv);
//        buffer.append(encryptedText);
        final String finalEncodedText = Base64.encodeToString(jsonArray.toString().getBytes("UTF-8"), Base64.NO_WRAP);

        return finalEncodedText;
    }

    private String decryptString(String textToDecrypt, String key) throws Exception {
        final byte[] firstDecodedBytes = Base64.decode(textToDecrypt.getBytes("UTF-8"),
                Base64.NO_WRAP);
        final String json = new String(firstDecodedBytes, "UTF-8");
        final JSONArray jsonArray = new JSONArray(json);
//        final byte[] encyptedBytes = Arrays.copyOfRange(firstDecodedBytes, 16, firstDecodedBytes.length);
//        final byte[] ivBytes = Arrays.copyOfRange(firstDecodedBytes, 0, 16);
        final String iv = jsonArray.getString(1);
        final String encyptedText = jsonArray.getString(0);
        Log.e(this.getClass().getSimpleName(), "DECRYPTION IV= " + iv);
        final String sha256 = CryptLib.SHA256(key, 32);
//        final String encyptedText = new String(encyptedBytes, "UTF-8");
        final String decryptedText = decrypt(encyptedText, sha256, iv);
